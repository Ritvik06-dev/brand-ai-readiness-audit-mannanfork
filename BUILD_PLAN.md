...
│   ├── access-discovery-audit/       ← SKILL.md + scripts/probe_access.py        (P0, snapshot-only reader)
│   ├── representation-parity-audit/  ← SKILL.md + scripts/analyze_representation.py (P0, snapshot-only reader)
│   ├── entity-consistency-audit/     ← SKILL.md + scripts/check_entities.py      (P0, snapshot-only reader)
...
| Cross-skill deps | **None, and no vendoring.** ALL network I/O (fetches, robots, UA/soft-404/redirect probes, sameAs/external-presence resolution) lives in `collect_snapshot.py`; every specialist script is a **snapshot-only reader**; a specialist's standalone mode is "run the collector first" (path via the composition fallback chain). No cross-skill imports or reads at runtime | PHASE1_REVIEW §3.3 — supersedes build review 1's vendoring |
...
7. **Dependency contract** (amended PHASE1_REVIEW §3.3): all network I/O — fetches, robots parsing, UA/soft-404/redirect probes, **and sameAs/external-presence resolution** (`external_presence[]`: url, declared_in, status, owned-vs-independent heuristic, brand_name_match) — lives in `collect_snapshot.py`; every specialist is a snapshot-only reader; standalone mode = "run the collector first". `compatibility:` is declared on skills that run scripts or touch the network (per the spec's own "most skills do not need it" note, SECOND_REVIEW §1), not on the three judgment-only skills.
8. **`check_catalog.json`** (PHASE1_REVIEW §2.2) — every `check_id` with `skill_id`, `category`, one-line `mechanism`, `evidence_required`, `severity_band`, **`negative_control`** (the when-NOT-to-flag a judge reads for), `affected_surfaces_default`. It is the join key for fragments → dedup → lint → playbook; **Phase 3 subagent drafters are gated on this file, not just the schemas**. (Playbook prose — options, owners, acceptance-test templates, forbidden claims — stays in Phases 5–7.)
9. **`excerpts_schema.json`** (PHASE1_REVIEW §2.3) — shapes for the three judgment I/O files: excerpt files (`audit/excerpts/<skill>.json`), `audit/passages.json` (answerability output), `audit/passages_checked.json` (collector `--passages` output; replaces the snapshot's `probes.fragment_contiguity`, which is removed — one home per artifact).

**Gate G1 (redefined so it can fail — PHASE1_REVIEW §6):** `tests/sample_fragment.json` validates against `finding_fragment.json` (real `jsonschema` as the dev-time oracle); stub `build_report.py` (stdlib-only, vendored draft-07 subset validator) converts it; the output validates against `output_schema.json`; the arithmetic guard passes; the whole suite runs under **Python 3.9** (`uv run --python 3.9 --with jsonschema tests/run_contract_tests.py`). Re-run and commit "Phase 1: G1 passed" before Phase 2 starts.

### Phase 2 — Snapshot collector + orchestrator spine (Day 2–4)
- `collect_snapshot.py`: safety preflight (http/https only, no credentials, private-range guard + `--allow-private`), robots fetch/parse (404 vs 5xx distinguished), bounded discovery → stratified 6–8 pages with template clustering, per-page capture (raw HTML, visible text, metadata, headings/landmarks, links+anchors, JSON-LD/microdata, tables/images/canvas/accordions/forms, timings), **claim index** (prices, dates, counts, plan names, versions, hours), bounded excerpts to `audit/excerpts/<skill>.json`, capability register, no-browser probes: soft-404 (`/<uuid>`), path-preserving redirects (http→https, www↔apex), robots-aware UA probes (homepage only, only if the probed token is allowed there — SECOND §2.8). **Also captures** (PHASE1_REVIEW §3.1–3.3): a `sitemap` object (parse outcome, entries, `lastmod` present/distinct counts + bounded sample) and per-page `sitemap_lastmod`; an allow-listed `headers` map per page and per probe (`server`, `cf-ray`, `cf-mitigated`, `last-modified`, `cache-control`, `content-language`, `link`); and `external_presence[]` (declared sameAs/footer links → status, owned-vs-independent heuristic, brand-name match) so no specialist ever does network I/O. Prints a small summary; the file is the artifact.
- `build_report.py`: read fragments → root-cause correlation candidates → assign F-IDs → severity-then-id stable sort → **arithmetic guard** (counts == len(findings); opportunities never counted) → schema validate → never-claim lint (**warns** into `lint_warnings[]`, never fails — quoted evidence or page prose can legitimately match trigger patterns) → human summary → exit codes. Schema validation uses a **vendored ~150-line draft-07 subset validator inside `build_report.py`** — `jsonschema` is NOT stdlib and the judge's box has no `uv`; real `jsonschema` stays as the dev-time oracle in `tests/` (PHASE1_REVIEW §2.4).
...
Each: SKILL.md (lean, mechanism sentences with vendor quote + URL in the body — SECOND §7) + script, fragment output only. **All specialist scripts are snapshot-only readers** of the one shared snapshot (PHASE1_REVIEW §3.3): no specialist performs network I/O; a specialist used alone starts by running the orchestrator's `collect_snapshot.py` (path via the composition fallback chain), or by asking the user to.
...
Parallelizable: the three SKILL.md drafts and script bodies can go to parallel subagents once G1's schemas **and `check_catalog.json`** are frozen — but the snapshot interface, fragment schema, and integration stay with the main thread.
...
- [ ] Vendored-slice sync item retired (PHASE1_REVIEW §3.3); replaced by: every `check_id` appearing anywhere under `skills/` resolves to `check_catalog.json` — no invented ids
- [ ] `grep -rn "Build status\|BUILD_PLAN" skills/ README.md` inside the zip root = empty (dev-phase markers don't ship)
- [ ] Every script-bearing skill declares `allowed-tools`; the Phase 7 README is generated from what actually ships
