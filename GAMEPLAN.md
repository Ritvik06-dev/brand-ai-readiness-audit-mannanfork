...
- [ ] `grep -rnE "GAMEPLAN|Build status|docs/(BUILD_PLAN|FINAL_APPROACH|INDEPENDENT_REVIEW|PHASE1_REVIEW|SECOND_REVIEW|README)|answerability|_fetch\.py" skills/ README.md` empty — no references to unshipped files or dead schemes (`_fetch.py` not bare `_fetch`, which false-positives on the `web_fetch` capability)
