...
Google's structured-data guidance treats markup as a description aid with quality rules
(developers.google.com/search/docs/appearance/structured-data/sd-policies) - which is why this
skill flags structured data that **contradicts** visible content or **fails to parse**, and
never flags merely missing or optional markup. The mistaken-identity risk it also measures is
the on-site half of cross-web agreement: when a brand name also denotes other prominent things
and the site carries no distinguishing qualifier, assistants resolve the name to the wrong
entity. Never require Wikipedia, Wikidata, or `@graph` - absence of optional markup is not a
defect.
...
4. **ENT-AMBIGUOUS-NAME** - *model judgment*. The script emits `gate: "pass"` with prepared
   ...
   title/H1/schema/about. The orchestrator's model completes the gate per the authoring rules
   below; a shared name alone is never a finding.
...
- When NOT to flag (negative controls), in plain words: valid JSON-LD without `@graph` and
  missing optional types pass; structured data scoped to a different offer or page region
  passes; a brand hierarchy with consistent legal naming passes; distinguishing qualifiers in
  title/H1/schema/about make a shared-name collision a pass; owned surfaces and unresolved
  fetches never manufacture corroboration findings.
