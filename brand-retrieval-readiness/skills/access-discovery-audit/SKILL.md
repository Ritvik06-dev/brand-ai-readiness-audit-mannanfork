...
- When NOT to flag: a 404 robots.txt; training-bot blocks (policy); a small comprehensively
  linked site without a sitemap; a canonical that merely normalizes scheme/host; challenge
  responses to abusive paths only; `noindex` on staging or duplicate URLs the site intends to
  hide; uniform sitemap `lastmod` without corroboration.
- Severity and confidence are separate; follow
  ...
  confidence. A failed or missing observation is `not_evaluated` with a reason, never a
  finding.
